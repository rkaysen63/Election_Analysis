...
Using a `with` statement I opened the csv file as `election_data` and used `csv.reader()` function to read the csv file in order to cycle through each row of the data.  The header row was removed from the data with the `next()` function. Then a `for` loop was set up to iterate through the data, row by row, to obtain the candidates' and counties' names and count the votes and voters, respectively, for each by increasing the total vote for each candidate as his/her name appeared in the data and similarly increasing the total number of voters for each county as the county name appeared in the list.  In other words, the total vote for the candidate was the number of times that candidate's name appeared in the data.  The total number of voters for a county was the total number of times the county name appeared in the data.  Within the `for` loop, two `if` statements were written.  The both `if` statements used a `not in` membership operator.  In the first `for` loop, `if` the candidate's name, `candidate_name`, was `not in` the candidate list, `candidate_options`, then the following occurred: the candidate's name was added to the list `candidate_options.append(candidate_name)` and the candidate's vote count was set to zero.  The second `if` statement using the `not in` membership operator performed the same function for the counties. `if county_name not in county_list:` then `county_list.append(county_name)`; `county_votes[county_name] = 0`.  Once the candidate or county was added to their respective lists, the vote count for candidates, or voter count for counties, was increased when that candidate or county name appeared in the list. e.g. `candidate_votes[candidate_name] += 1`

A `with` statement was used to open the file to save and write to it `with open(file_to_save, "w") as txt_file:`.  An `election_results` variable was created containing an f-string combining text and {total votes:,} that was used to print the total votes to the terminal `print(election_results, end="")` and write the total votes to the text file `txt_file.write(election_results)`.  A `for` loop was set up retrieve each county's voter count, calculate the percentage of voters per county, print the county results and write the county results to a text file.  The percentage of voters = county voter count divided by the total number of voters times 100.  `votesCo_percentage = float(votesCo) / float(total_votes) * 100`, where `float()` is used to convert the integer to a floating decimal point value.  Within this `for` loop there is an `if` statement to determine the county with the largest voter turnout. `if (votesCo > county_voter_turnout):` then `largest_county = county_name` and `county_voter_turnout = votesCo`.  The `if` statement says that if the total number of voters for a county is greater than the county voter turnout, then that county will be considered the largest county (at least for a time). The county voter turnout was initialized to zero before the data was loaded, `county_voter_turnout = 0`. The first county to be read will meet `if` statement criteriat even if it only has one voter and it's total vote will now take the place of zero for `county_voter_turnout`. The next county will be evaluated to see if it meets the criteria, which has changed from zero to the first county's voter count, and it will either take the place of the first county because it has more voters, or it will not and will be passed over.  Then the nexct county will be evaluated, and so on until all the counties are reviewed.  After the `for` loop is completed, then the `largest-county_summary` was created with an f-string and the `{largest_county}` name to print and write in the analysis.

Similarly, a `for` loop with an `if` statement for the candidates' results was set up to retrieve the number of votes each candidate received, calculate the percentage of votes for each candidate and to determine the winner by popular vote and to print and write the results to the terminal and text file, respectively.
